...
pub struct BufWriter<W: Write, A: Allocator = Global> {
    inner: W,
...
impl<W: Write> BufWriter<W> {
    ...
    pub fn with_capacity(capacity: usize, inner: W) -> Self {
        Self::with_in(Vec::with_capacity(capacity), inner)
    }

    pub fn with_capacity_aligned<Align>(capacity: usize, inner: W) -> Self {
        Self::with_in(unsafe { aligned_vec::<Align>(capacity) }, inner)
    }
}

impl<W: Write, A: Allocator> BufWriter<W, A> {
    pub fn with_in(buf: Vec<u8, A>, inner: W) -> Self {
        Self {
            inner,
            buf,
            panicked: false,
    ...
    fn flush_buf(&mut self) -> io::Result<()> {
        ...
        struct BufGuard<'a, A: Allocator> {
            buffer: &'a mut Vec<u8, A>,
            written: usize,
        }

        impl<'a, A: Allocator> BufGuard<'a, A> {
            fn new(buffer: &'a mut Vec<u8, A>) -> Self {
        ...
        impl<A: Allocator> Drop for BufGuard<'_, A> {
            fn drop(&mut self) {
                if self.written > 0 {
    ...
    pub fn buffer_mut(&mut self) -> &mut Vec<u8, A> {
        &mut self.buf
    }
...
impl<W: Write, A: Allocator> Write for BufWriter<W, A> {
    #[inline]
...
impl<W: Write, A: Allocator> fmt::Debug for BufWriter<W, A>
where
    W: fmt::Debug,
{
...
impl<W: Write + Seek, A: Allocator> Seek for BufWriter<W, A> {
...
impl<W: Write, A: Allocator> Drop for BufWriter<W, A> {
    fn drop(&mut self) {
        if !self.panicked {
