...
pub struct Buffer<A: Allocator = Global> {
    // The buffer.
    buf: Box<[MaybeUninit<u8>], A>,
...
impl Buffer {
    ...
    #[inline]
    pub fn with_capacity_aligned<Align>(capacity: usize) -> Self {
        let buf = unsafe {
            let (ptr, _, cap) = aligned_vec::<Align>(capacity).into_raw_parts();
            let slice = slice::from_raw_parts_mut(ptr as *mut MaybeUninit<u8>, cap);
            Box::from_raw(slice)
        };
        Self::with_in(buf)
    }
...
impl<A: Allocator> Buffer<A> {
    pub fn with_in(inner: Box<[MaybeUninit<u8>], A>) -> Self {
        Self {
            buf: inner,
            pos: 0,
            filled: 0,
